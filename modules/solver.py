...
class Problem():
    ...
    def set_Tasks(self, tasks: TS.Tasks):
        self.__tasks = tasks
        return None
    ...
    def set_Workers(self, workers: WS.Workers):
        self.__workers = workers
        return None
    ...
    def get_Tasks(self) -> TS.Tasks:
        return self.__tasks
    
    def get_Bases(self) -> BA.Bases:
        return self.__bases
    
    def get_Workers(self) -> WS.Workers:
    ...
    def solve(self, which_solver:str) -> bool:
        ...
        Solves an existing problem with any of the implement formulations / methods. As of now:
            - "abstract_DFJ": It uses an adaptation of the DFJ subtour elimination constraints. Too Slow to be used.
            - "abstract_dynamic_DFJ": It is the same as the previous one but instead of adding all the subtours constraints
                from the start, it adds them dynamically if broken. It is usually way faster but somewhat less stable.
        """
...
def abstract_DFJ_Solver(problem: Problem) -> bool:
    ...
    # This does not duplicate in memory, just "reference" it.
    # This is just default python. Just for faster coding and less referencing

    ptowers = problem.get_Tasks() 
    pbases = problem.get_Bases()
    puavs = problem.get_Workers()
